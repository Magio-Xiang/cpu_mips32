...
- ID模块 
    ...
    | mem_wd_i    |5            |输入           |访存阶段的指令要写入的目的寄存器地址        | 
    | mem_wreg_i  |1            |输入           |访存阶段的指令是否要写入目的寄存器          | 
    | mem_wdata_i |32           |输入           |访存阶段的指令要写入目的寄存器的值          | 
    | ex_wd_i     |5            |输入           |执行阶段的指令要写入的目的寄存器地址        | 
    | ex_wreg_i   |1            |输入           |执行阶段的指令是否要写入目的寄存器          | 
    | ex_wdata_i  |32           |输入           |执行阶段的指令要写入目的寄存器的值          | 
